...
client.on("ready", () => 
{
	...
	// 	{
	// 		let voters = client.guilds.get("546975300261511168").roles.find(r => r.name === "Voters");
	// 		client.channels.get("611481638353895424").send("<@&" + voters.id + "> voting has reset! https://mapleunity.com/vote");
	// 	}
	...
	// 			{
	// 				client.channels.get("619090046699700254").send("<@" + row.userID + ">'s shop will expire in 1 hour!");
	// 				sql.run('UPDATE shop SET notified = 1');
	// 			}

	// 			if (diffMs <= 0)
	// 			{
	// 				client.channels.get("619090046699700254").send("<@" + row.userID + ">'s shop has expired!");
...
client.on("message", (message) => 
{
	...
	{
		case "help":
			message.channel.send("**How to use: Type a command with ! before it.**\n\n" +
		...
		case "roll":
				...
				else
				{
					...
					else
					{
						...
						else if (num == 100)
						{
							...
							var role100 = message.guild.roles.find(r => r.id === "613585947329691676");
			...
			{
				sql.run('CREATE TABLE IF NOT EXISTS roll (lastID VARCHAR (30))').then(() => 
				{
					sql.run('INSERT INTO roll (lastID) VALUES ("")');
				}).catch(error =>
				{
		...
		case "timer":
			if (args.length == 3)
			{
				...
				if (boss == "bf" || boss == "hh")
				{
					...
					{
						sql.run('CREATE TABLE IF NOT EXISTS timers (timerID INTEGER PRIMARY KEY, channel INTEGER, boss VARCHAR(20), time VARCHAR(40), map VARCHAR(20))').then(() => 
						{
							sql.run('INSERT INTO timers (timerID, channel, boss, time, map) VALUES (0, 0, "", "", "")');
			...
			else if (args.length == 1)
			{
				...
				else
				{
					...
					{
						console.log(err);
						sql.run('CREATE TABLE IF NOT EXISTS timers (timerID INTEGER PRIMARY KEY, channel INTEGER, boss VARCHAR(20), time VARCHAR(20), map VARCHAR(20))').then(() => 
						{
							sql.run('INSERT INTO timers (timerID, channel, boss, time, map) VALUES (0, 0, "", "", "")');
							console.log('created table');
						}).catch(error =>
		...
		// 	if (isNaN(total) || isNaN(ways))
		// 	{
		// 		message.channel.send("Arguments must be two numbers.");
		// 		return;
		// 	}

		// 	var splits = Math.round(total / ways);
		// 	//var items = Math.floor(splits / 49999);
		// 	//var remain = (splits % 49999);

		// 	message.channel.send("Splitting **" + total.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + " **" + ways + "** ways" + "**\n" +
		...
		case "curry":
			...
			{
				sql.run('CREATE TABLE IF NOT EXISTS curry (ch INTEGER)').then(() => 
				{
					sql.run('INSERT INTO curry (ch) VALUES (0)');
				}).catch(error =>
				{
					console.log("Error: " + error.message);
				});
		...
		// case "vote+":
		// 	message.member.addRole(message.guild.roles.find(r => r.name === "Voters"));
		// 	message.channel.send(message.author + " has been added to the list of voters.");
		// 	break;

		// case "vote-":
		// 	message.member.removeRole(message.guild.roles.find(r => r.name === "Voters"));
